...
class AdvertisementController extends Controller {
    ...
    protected function grid() {
        ...
        $grid->title('轮播图标题');
        $grid->img_uri('轮播图')->display(function ($value) {
...
//        $grid->divided_amount('被分润金额');
        ...
        $grid->tools(function ($tools) {
            $tools->batch(function ($batch) {
                $batch->add('隐藏', new ChangeAdvertisementStatus(0));
                $batch->add('显示', new ChangeAdvertisementStatus(1));
            });
        });
    ...
    protected function detail($id) {
        ...
        $show->title('标题');
        $show->img_uri('轮播图')->as(function ($value) {
            return $value ?: $this->img;
        })->image();
//        $show->ad_expenses('轮播图经费');
...
//        $show->divided_amount('被分润金额');
    ...
    protected function form() {
        ...
        $form->text('title', '轮播图标题')->rules('required');
//        $form->decimal('ad_expenses', '广告经费')->rules('required');
        $form->image('img', '轮播图文件')->rules('required_without_all:img_uri');
